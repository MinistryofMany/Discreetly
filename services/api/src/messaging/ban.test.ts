...
beforeAll(async () => {
  const r = await prisma.room.create({ data: {
    name: 'Ban Test', slug: `ban-${Date.now()}`, rlnIdentifier: `${ctx.rlnIdentifier}`,
    rateLimit: 10_000, userMessageLimit: 1, maxDevices: 5, accessPolicy: { badge: { type: 'x' } },
  }});
...
describe('banOnCollision', () => {
  it('recovers the secret, bans the membership, prunes all leaves, records a Ban', async () => {
    ...
    const seated = await joinRoom({ room, joinNullifier, identityCommitment: ctx.identity.commitment.toString() });
    ...
    const membership = await prisma.membership.findUnique({ where: { roomId_joinNullifier: { roomId: room.id, joinNullifier } } });
    expect(membership?.status).toBe(MembershipStatus.BANNED);
    const remainingLeaves = await prisma.membershipLeaf.count({ where: { membershipId: membership!.id } });

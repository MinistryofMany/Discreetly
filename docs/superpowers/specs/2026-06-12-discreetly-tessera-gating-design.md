...
1. **One membership per Tessera identity, per room.** Gated *by* badges, but uniqueness is anchored on the Tessera identity, not on any badge (re-issuing a badge must not grant a second membership).
...
- Inherent consequence (accepted): a user's successive rotations and device-leaves are linkable *to each other* under one pseudonym at the server — unavoidable given "ban must persist across rotation."
